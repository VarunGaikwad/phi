...
> Status: VM/container isolation and reviewed working copies are approved. Windows 11 Home, Docker Desktop/WSL2, and mainly Node.js development are confirmed. Phase 0 diagnostics and an opt-in [Docker fixture prototype](Docker-Prototype.md) are implemented. After explicit image-download approval, the [native Windows Node fixture](Windows-Docker-Fixture-Pass.md) passed all ten checks and cleanup on Engine 29.4.3 / Desktop Linux / WSL2. A separate [host-only synthetic-copy fixture](Synthetic-Copy-Prototype.md) passed seven checks and cleanup; it is not production admission or race-safety proof. Provider/gateway work is deferred at the operator's request, not passed. The [offline whole-agent fixture](Offline-Agent-Prototype.md) has now [passed all 20 checks on native Windows](Windows-Offline-Agent-Run.md), including real SDK tools/inline extension, synthetic containment, a detached child, single reload, host stop and verified cleanup. A separate [controlled worker-loss primitive](Worker-Loss-Prototype.md) also passed five checks and cleanup on Windows: the guest kept running after worker death until the surviving supervisor stopped it. A separate [synthetic network/IPC primitive](Network-IPC-Prototype.md) passed 21 checks and both cleanups, including guest-local DNS/redirect controls, TEST-NET egress denials and live peer isolation. The offline-agent fixture was then [expanded with lifecycle checks](Agent-Lifecycle-Prototype.md) and passed **35 checks** with independent cleanup: synthetic guest-only new/resume/fork/clone/tree/reload, cancellation with explicit queue clearing, and extension tool/hook/shutdown failures. These are fixed-experiment results only; packet-level/real-host network proof, supervisor/daemon-loss recovery, broader lifecycle and production backend acceptance remain unproven. A subsequent [restart-reconciliation design and pure model](Restart-Reconciliation-Design.md) now tests journal/cleanup decisions offline; no production durable store, engine adapter or watchdog is implemented. The subsequent [Windows synthetic store/fake-engine fixture](Windows-Store-Prototype.md) passed **15 checks** with confirmed worker exit and independent absence of all six temp roots. Native file sharing is real; injected faults and engine receipts are synthetic, not power-loss durability or Docker fencing. The subsequent [command-owner/receipt design and in-memory experiment](Command-Owner-Receipts-Design.md) passes 24 new tests; late work survives simulated owner replacement, and its receipt oracle is explicitly mocked, not a proven Docker capability. The subsequent [installed Docker CLI/API evidence review](Docker-Recovery-API-Review.md) leaves actual incarnation/receipt semantics unverified: matching API source/schema was not located in the checked local material, so a recovery adapter remains blocked. The subsequent [Windows security API/memory review](Windows-Private-Storage-IPC-Review.md) passes 12 new tests but proves no kernel privacy or IPC authentication. The separately reviewed [file-only ACL fixture](Windows-File-ACL-Prototype.md) then passed **10 native checks** with both workers exited and independent temp absence; creation-time DACL/collision and same-user read controls are not cross-user/process isolation or production privacy. The subsequent [installed pipe implementation review](Windows-Pipe-Contract-Review.md) records 219 bodies and passes 13 new offline tests: Framework's one-instance constructor adds a first-instance bit despite the missing enum member, but explicit local-only rejection and exact peer/process-lifetime binding remain unestablished. No listener/service was added. No fixture or model completes Phase 0. See [Execution Order](Execution-Order.md) and [Phase 0 Findings](Phase-0-Findings.md).
>
...
Do not ask again whether VM/container use or reviewed working copies are acceptable. The operator has now supplied Windows 11 Home, Docker Desktop and WSL2 installed, and mainly Node.js development. The first native Windows run has recorded the build, installed Desktop/CLI and WSL versions, and non-elevated process status. The retry has now observed running Engine 29.4.3 / Desktop Linux / WSL2 metadata. The operator explicitly approved the Node image download, and the primitive fixture has now passed its ten checks and cleanup. Remaining discovery concerns active settings/constraints; the fixed offline whole-agent launch now passes, but the complete containment boundary remains unproven. Provider/authentication-method and gateway work is now deferred by the operator; keep it pending while continuing offline work without requesting those details again. The host-only synthetic-copy experiment has passed, but does not prove integrated admission or NTFS race safety. After initial engine blockers, the unchanged offline-agent fixture passed all 20 combined checks on native Windows, including real SDK execution, host-owned stop, source/canary verification and cleanup. A separate query confirmed the container absent. The subsequent worker-loss and synthetic network/IPC primitives also pass their fixed scopes. The expanded whole-agent session/cancellation/extension-failure sequence now passes 35 combined checks with independent cleanup. The durable ownership/restart-reconciliation design and pure model are now tested offline. Windows API review and the disposable synthetic store/fake-engine harness now pass their limited scope (15 native checks, confirmed cleanup). Single command-owner / uncertain-operation receipt design and the deterministic fake-engine experiment now pass 24 tests. The installed CLI/API evidence review now records 11 offline help/version calls, but required Engine identity/receipt semantics remain unverified; no matching API source/schema was located in the checked material. Keep the recovery adapter blocked, never fabricate receipts. Windows private-storage/local-IPC API review and synthetic memory descriptors now pass 12 tests; newer named pipe options are absent from the actual Framework enum. The file-only creation-time ACL fixture now passes 10 native checks with independent cleanup, without existing ACL/settings changes, pipe listeners, process kills or Docker integration. Next is reviewing a supported native pipe namespace/local-only/peer-binding contract before an IPC harness. Same-user controls must not be reported as cross-user/process isolation. Private production storage, durable checkpoint recovery and real fencing still precede any supervisor-loss test; actual restart recovery, broader lifecycle and network variants remain pending. No production importer or supported backend is established by these results. No need to share private-file contents or credentials for this discovery.
